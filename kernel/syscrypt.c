...
uint64 sys_map_shared_pages(void)
{
  ...
  int dest_pid;
	...
	struct proc* dest_proc = find_proc(dest_pid);
	if (dest_proc == 0)
	{
		return -1;	
	}
		
	uint64 dst_va = map_shared_pages(src_proc, dest_proc, src_va, size);
	if (dst_va == 0)
	{
		return -1;
	}
...
uint64 sys_unmap_shared_pages(void)
{
  int dest_pid;
	...
	uint64 ret = unmap_shared_pages(dest_proc, dest_va, size);
	// if (dst_va == 0)
	// {
	// 	return -1;
	// }
